...
1. How do annual members and casual riders use Cyclistic bikes differently?
  ...
  By looking at the graph, it seems like members use the bikes to commute to work because Monday to Friday number of rides are higher compared to the weekend which where most businesses are closed. 
  In addition, duration of the ride are shorter which is possibly because of work schedule. In contrast, non-members might only use the bike for recreational purpose because the rides are higher on the weekend and not the weekdays. 
...
Few limitations:
  ...
  For instance, we might need additional dataset that shows relationship between age and gender and digital media use. However, those two variables, gender and birth year, are not complete or have empty values which will not provide an accurate answer.
